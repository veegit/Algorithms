...
public class BST<M extends Object> implements Comparator<M> {
	   private TreeNode<M> root;
	   public static enum Traverse {
			INORDER,PREORDER,POSTORDER;
		}
	   
	   public BST(M data) {
	       root = new TreeNode<M>(data);
	   }
	   ...
	   public TreeNode<M> insert(M data) {
	    TreeNode<M> newNode = new TreeNode<M>(data);
	   ...
	   public int[] getParentsData(TreeNode<M> node) {
		   ...
		   /** DONT do node.getParent != null
		    *  Otherwise you may fail to include that element itself in its parent
		    */
		   while(node!=null) {
	   ...
	   public static void main(String args[]) {
		  /* BST<Integer> tree = new BST<Integer>(10);
		   Random generator1 = new Random( 19580427 );
			int cnt1,max1;
			cnt1 = 1000;max1 =cnt1;
			while(cnt1-- > 0)
				tree.insert(cnt1);
		   ...
		   System.out.println();
		   /*System.out.println(bst.LCA(node1, node2).getData());
		   bst.delete(5);
		   
		   System.out.println();
		   
		   int max = 1000000;
		   BST<Integer> bst1 = new BST<Integer>(max);
		   Random generator = new Random(102134);
		   TreeNode<Integer> nodex = null;
		   TreeNode<Integer> nodey = null;
		   while(max-- > 50000)  {
		   ...
		   int[] nodexarr = bst1.getParentsData(nodex);
		   int[] nodeyarr = bst1.getParentsData(nodey);
		   for (int i = 0; i < nodexarr.length; i++) 
			   System.out.print(nodexarr[i] + " ");
		   System.out.println();
		   for (int i = 0; i < nodeyarr.length; i++) 
			   System.out.print(nodeyarr[i] + " ");
		   System.out.println();
		   System.out.println(bst1.LCA(nodex, nodey).getData());*/
		}
	  }