...
public class QuickSort <M> extends BaseSort<M> implements Comparator<M>{
	...
	public int size;
	public int cnt=0;
	public enum PIVOT {FIRST, LAST, MIDDLE, MEDIAN, RANDOM, FIRST_2,MIDDLE_2};
	...
	public M[] quickSort(M[] a, PIVOT pivot) {
		...
		cnt=0;
		switch(pivot) {
			case FIRST: runQuickSort_First(0,size - 1); break;
			case LAST: runQuickSort_Last(0,size - 1); break;
			case MIDDLE: runQuickSort_Middle(0,size - 1); break;
			case MEDIAN: runQuickSort_Median(0,size - 1); break;
			case RANDOM: runQuickSort_Random(0,size - 1); break;
			case FIRST_2: runQuickSort_First_2(0,size - 1); break;
			case MIDDLE_2: runQuickSort_Middle_2(0,size - 1); break;
			default: runQuickSort_First(0,size - 1); 
		}
		System.out.println("# of comparisons " + cnt);
		return array;
	}
	...
	void runQuickSort_Middle(int low, int high){
		if(low >= high)
			return;
		...
		int index = partition(low,high);
		cnt = cnt + high-low;
		runQuickSort_Middle(low,index-1);
		runQuickSort_Middle(index+1,high);
	}
	...
	private void runQuickSort_Middle_2(int low, int high) {
		cnt = cnt + high-low;
		int i = low, j = high;
		...
		// Recursion
		if (low < j)
			runQuickSort_Middle_2(low, j);
		if (i < high)
			runQuickSort_Middle_2(i, high);
	}

	void runQuickSort_First_2(int low, int high){
		if(low >= high)
			return;
		int index = partition_2(low,high);
		cnt = cnt + high-low;
		runQuickSort_First_2(low,index-1);
		runQuickSort_First_2(index+1,high);
	}
