...
public class Inversions<M> implements Comparator<M> {
	...
	private void mergeANDcount(int low, int middle, int high) {
		...
		while (i <= middle && j <= high) {
			if (compare(helper[i], helper[j]) <= 0) {
				numbers[k] = helper[i];
				i++;
				// System.out.println(String.format("\tnum=%d, i=%d,
				// middle+1=%d, j=%d, num[i]=%d, num[j]=%d", num, i, middle+1,
				// j, helper[i-1], helper[j]));
				 ...
				 */
				// System.out.println(String.format("num=%d, i=%d, middle+1=%d,
				// j=%d, num[i]=%d, num[j]=%d", num, i, middle+1, j-1,
				// helper[i], helper[j-1]));
				num = num + middle + 1 - i;
			}
			k++;
		...
		/*
		 * Why not for right? = since the right side if greater is already
		 * placed correctly in the numbers array
		 */
		while (i <= middle) {
			numbers[k] = helper[i];
			k++;
	...
	public static void main(String args[]) {
		...
		}
		{
			...
			try {
				scanner = new Scanner(
						new InputStreamReader(inv.getClass().getClassLoader().getResourceAsStream("IntegerArray.txt")));
